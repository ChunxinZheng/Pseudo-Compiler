...
During the first scanning stage of the program, a table that maps each variable to the address in the stack relative to ```fp``` where the value of such variable may be stored in during a function applicaiton. <br>
...
The rest of the function definition is compiled as usual statements with all occurrences of variables are replaced by their addresses relative to ```fp``` based on the previously generated table.
